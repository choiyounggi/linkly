...
# 어휘(동사·키워드·타입·절·집행·진단·예약어) 렌더러 넷(grammar/verbs/declarations/
# types)의 정본은 더 이상 컴파일러 상수를 직접 참조하지 않는다 — issue #135의
# 단일 소스 `vocabulary_document()`(impl/lnpl/vocab.py)를 부른다. naming.md/
# spec.md/rfcs.md는 어휘가 아니라 이름 파생 규칙·spec 문법·RFC 라우팅을 다뤄서
# 이 셋의 범위 밖이다 — 그 렌더러들은 위 import(KIND_PREFIX 등, EXPECTATIONS 등)를
# 계속 직접 쓴다.
VOCAB = vocabulary_document()
...
def render_grammar():
    ...
    lines.append("## 최상위 선언\n")
    lines.append(" ".join("`%s`" % k for k in kw["top"]) + "\n")
    lines.append("## 절(clause)\n")
    lines.append(" ".join("`%s`" % k for k in kw["clause"]) + "\n")
    lines.append("## 제어 어휘\n")
    lines.append(" ".join("`%s`" % k for k in kw["control"]) + "\n")
    lines.append("## 예약어 — 사용 불가\n")
    lines.append(" ".join("`%s`" % k for k in VOCAB["reserved"]) + "\n")
    lines.append("이 넷은 **문법적으로 표현 불가능**하다. 쓰면 렉서가 거부한다. "
                 "분기가 필요하면 `when`, 반복이 필요하면 `repeat`/`until`을 쓴다.\n")
    lines.append("## 리터럴\n")
    lines.append("기간 단위: " + " ".join("`%s`" % u for u in kw["duration_units"]))
    lines.append("\n비교 연산자: " + " ".join("`%s`" % c for c in kw["comparators"]) + "\n")
    lines.append("## 값 표현식 (RFC-0015)\n")
    lines.append("산술 연산자: " + " ".join("`%s`" % o for o in kw["arithmetic_operators"]))
    lines.append("\n논리 결합: " + " ".join("`%s`" % o for o in kw["logical_operators"])
                 + " — `or`·`not`·괄호는 없다.")
    lines.append("\n할당: " + " ".join("`%s`" % k for k in kw["assign_keywords"])
                 + " (`set <바인딩>.<필드> to <값>`)")
    lines.append("\n입력 네임스페이스: `%s` (`input.quantity` — 실행 payload의 필드)\n"
                 % kw["payload_namespace"])
    ...
    guard_alt = kw["guard_alt_keyword"]
    ...
    verbs = VOCAB["verbs"]
    read_verbs = kw["read_verbs"]
    reads = list(read_verbs)
    create_verbs = [v for v, meta in verbs.items()
                    if meta["effect"] == "RepositoryCall"
                    and meta["attrs"].get("operation") == "create"]
    writes = [v for v, meta in verbs.items()
             if meta["effect"] == "RepositoryCall"
             and meta["attrs"].get("operation") in ("update", "delete")]
    rowset_verbs = [v for v, meta in verbs.items()
                    if meta["effect"] == "RepositoryCall" and v not in read_verbs
                    and meta["attrs"].get("operation") not in ("create", "update", "delete")]
    ...
    closers = [k for k in kw["control"] if k != "merge"]
    lines.append("\n## 블록의 시작과 종결\n")
    ...
    lines.append("- `on %s <주기> %s <HH:MM> <존>` — 주기: %s / 존: %s"
                 % (kw["schedule_keyword"], kw["schedule_at"],
                    " ".join("`%s`" % r for r in kw["schedule_recurrences"]),
                    " ".join("`%s`" % z for z in kw["schedule_zones"])))
...
def render_declarations():
    clauses = VOCAB["clauses"]
    lines = ["선언했다고 집행되는 것이 아니다. 아래 표가 정본이다 — "
             "`enforced`만 실행을 바꾼다. `measured`는 관측·보고하되 막지 않고, "
             "`unenforced`는 런타임이 완전히 무시한다(issue #38).\n",
             "## 절별 허용 이름\n",
             "- `policy`: " + " ".join("`%s`" % n for n in clauses["policy"]),
             "- `security`: " + " ".join("`%s`" % n for n in clauses["security"]),
             "- `performance`: " + " ".join("`%s`" % n for n in clauses["performance"]),
             "",
             "인자를 받는 security 기제: " +
             (" ".join("`%s`" % n for n in clauses["security_argument_mechanisms"]) or "—"),
             "값 없이 쓰는 performance 지표: " +
             (" ".join("`%s`" % n for n in clauses["performance_valueless"]) or "—"),
             "",
             "## 집행 매트릭스\n",
             "| 선언 | 상태 | 런타임이 실제로 하는 일 |",
             "|------|------|--------------------------|"]
    for rec in VOCAB["enforcement"]:
        lines.append("| `%s %s` | **%s** | %s |"
                     % (rec["clause"], rec["name"], rec["status"], rec["why"]))
    lines.append("\n## 진단 코드\n")
    ...
    lines.append("| 코드 | 등급 |")
    lines.append("|------|------|")
    for d in VOCAB["diagnostics"]:
        lines.append("| `%s` | **%s** |" % (d["code"], d["severity"]))
    lines.append("")
...
def render_types():
    ...
    lines.append("|------|------------|")
    for base in types["base_category"]:
        facets = types["category_facets"][types["base_category"][base]]
        lines.append("| `%s` | %s |" %
...
RFC_ROUTES = {
    ...
    "0030": ("`create <명사> as <이름>`로 생성 직후 그 행에 `set`/`format`/"
             ...
             "정확히 무엇이 바이트 동일한지", ()),
    "0031": ("서비스 하나를 여러 `.lnpl` 파일로 나누고 싶다 — 파일을 여러 개 "
             "주거나 디렉터리를 줄 때 병합 순서가 무엇으로 정해지는지, 이름이 "
             "겹치면 왜 두 파일:줄이 함께 나오는지, 파일 하나만 줄 때 왜 "
             "여전히 바이트 동일한지", ()),
    ...
    "0033": ("(Draft) 서로 다른 도메인 디렉터리에서 같은 엔티티 이름(`Order` "
             "등)을 쓰고 싶다 — 디렉터리가 어떻게 네임스페이스가 되는지, "
    ...
    "0042": ("확장이 자기 진단 코드를 등록하고 싶다 — `<prefix>/<code>`의 "
             "prefix가 왜 반드시 엔트리포인트 등록명 그 자체인지, 왜 `lnpl`·"
             "`core`가 예약어이고 bare 코드가 영구히 코어 전용인지, 확장이 왜 "
             "`error`를 선언할 수 없는지, `--strict`가 왜 확장 코드는 기본으로 "
             "건너뛰는지, 확장 진단기가 왜 소스 텍스트를 못 보고 컴파일된 IR과 "
             "자기 설정만 보는지", ()),
    "0043": ("설치된 드라이버가 자기 전달 보증·격리 수준·캐시 스코프·토큰 "
             "클레임을 신고해 `lnpl compile` 진단으로 내고 싶다 — "
             "`lnpl_enforcement` 신고 SPI가 무엇을 받는지, capability 선언과 "
