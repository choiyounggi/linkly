...
class DuplicateUrlError(Exception):
    """M2 (issue #142 edit-task benchmark): reject a second bookmark for the
    same url, independent of id. No LNPL analogue — see
    ../edits/m2_duplicate_guard/lnpl_after.lnpl's header comment: LNPL guards
    can't reference a Text field, and there is no way to test "does a row
    with this url exist" without a hard-failing read, so the equivalent
    native LNPL edit ends up rejecting every save instead. This class is
    exactly the asymmetry REPORT.md documents.
    """
...
class Store:
    ...
    url_index: dict = field(default_factory=dict)  # url -> bookmark id
...
def create_bookmark(store: Store, payload: BookmarkCreate) -> Bookmark:
    ...
    # M2: reject a second save of a url already in the store, regardless of id
    if payload.url in store.url_index:
        raise DuplicateUrlError(f"bookmark url already saved: {payload.url}")
    row = Bookmark(
        id=payload.id,
        url=payload.url,
        title=payload.title,
    ...
    store.url_index[row.url] = row.id
    return row
