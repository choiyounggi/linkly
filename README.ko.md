...
**테스트 2474개 전부 통과**, 그리고 그 스위트가 실제로 실패할 수 있음을 증명하는
77종 뮤테이션 하네스. 둘 다 [검증](#검증)의 명령으로 재현한다.

**RFC 32편 — 31편 `Accepted`, RFC-0000은 RFC-0007로 `Superseded`.** RFC-0007은
...
| [0032 트랜잭션 경계와 rollback 집행 승격](rfcs/0032-transaction-boundary-and-rollback-enforcement.md) | 워크플로 실행 하나가 암묵적 트랜잭션 하나가 된다(명시적 `Transaction` IR 노드는 아직 없다): 성공 시 커밋, 실패 시 롤백 — 실패한 실행이 등록한 이벤트 발행도 함께 되돌아간다. `policy rollback`이 `unenforced`에서 `enforced`로 승격된다. *0003 §Execution Model·§Policy Enforcement·§Examples 갱신* |
...
rfcs/0000~0029              RFC (0000은 Superseded, 나머지 29편은 Accepted)
