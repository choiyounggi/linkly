...
| B3 | `lnpl-spec`의 실제 내용 | 어휘 나열이 아니라 **선언 → 기대 도출 규칙**: `retry N` → `attempts N+1`, `timeout` → 데드라인 케이스, `response` → `slo met`, `cache` → `cache written`, 정상 케이스 1건은 항상 포함 | 어휘는 이미 생성물 `references/spec.md`에 있다. 스킬이 그걸 반복하면 A3(정본은 소스)을 깨고 drift를 만든다. 더할 값은 도출 규칙이고, 그건 `plans/agent-roles` A5가 Tester에 이미 코드로 넣은 규칙이라 **구현과 일치가 보장된다** |
