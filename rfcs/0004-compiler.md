...
- **M3 신규 노드 추가** — 스키마 19 kind 중 하나이며 신규 `id`를 받는다.
...
| **Heap** | workflow 실행 수명(= arena 수명)을 넘겨 생존해야 하는 값. 소유권이 외부 계층으로 이전되는 값, 비동기 발행 이후에도 읽히는 페이로드가 해당한다. **단 heap 프리미티브 자체의 런타임 계약(할당·해제 책임, 수명 종료 시점)은 v0.1에서 미정이다** — RFC-0003 §Memory Model의 프리미티브 계약 표에는 arena·pool 2행만 있고 heap 행이 없다. 이 행이 다루는 것은 **배치 분류뿐**이며, heap 프리미티브의 런타임 계약은 RFC-0003 소유의 미해소 항목이다(`docs/CONSISTENCY-CHECK.md` C8) |
