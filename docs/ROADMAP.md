...
- ~~**RFC Status는 전부 `Draft`다.**~~ → **2026-07-31: 0001~0006 전부 `Accepted`.** 교차 정합성
  전항 PASS + 소유자 승인으로 승격됐다(근거 `docs/CONSISTENCY-CHECK.md`). 이후 개정은 본문 편집이
  아니라 RFC-0007 §2.2의 두 관계로 한다 — 전면 대체(Supersedes) 또는 절 단위 갱신(Updates).
  RFC-0000은 RFC-0007로 대체돼 `Superseded`다.
...
| **R29** | 표현식 표기 미정으로 `BusinessRule.expression`이 v0.1에서 비어 있다(`GoalLine`은 `name`=`statement`만 채운다) | RFC-0002 OQ ③ + 부록 A.5. (OQ ②는 2026-08-02 해소 — RFC-0009. 남은 근거는 ③뿐이다. 줄 번호 인용은 드리프트해 제거했다) | KB 문서가 지시하는 비즈니스 규칙을 IR의 형식 표현으로 옮길 수 없다. `statement` 산문으로만 남는다 |
| ~~**R30**~~ | ~~가드 조건식의 표현력 한계 — `Condition`이 비교식 + 1~4토큰 구가 전부~~ → **해소됨 (2026-08-02)**. `Condition`은 `Presence \| Comparison`이다(RFC-0008). 평가기 없던 1~4토큰 구 대안은 제거됐고 파서가 파스 시점에 거부한다 | RFC-0009 (`Updates: RFC-0002 §Open Questions`) — 결정 내용은 그쪽이 정본 | 남은 확장 여지는 둘로 갈린다: 부정·논리 결합은 **RFC-0008 OQ 3**, 멤버십 검사는 **RFC-0002 OQ ②**(RFC-0009 갱신본)가 계속 추적한다 |
...
| | ~~② 가드 조건식의 표현력~~ | **해소 (2026-08-02)** — RFC-0008 + RFC-0009. 멤버십 검사만 잔여 |
